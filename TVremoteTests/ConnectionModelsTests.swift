...
final class ConnectionModelsTests: XCTestCase {
    ...
    func testConnectionState_displayText() {
        ...
        let failedPairing = ConnectionState.pairing(.failed("bad pin"))
        XCTAssertTrue(failedPairing.displayText.contains("bad pin"))
        XCTAssertTrue(failedPairing.displayText.contains("Pairing failed"))
